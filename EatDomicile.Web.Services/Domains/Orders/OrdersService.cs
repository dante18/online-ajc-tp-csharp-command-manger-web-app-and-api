...
public class OrdersService : IApiOrdersService
{
    ...
    {
        var response = await httpClient.PostAsJsonAsync("https://localhost:7001/api/orders", orderDTO);
        _ = response.EnsureSuccessStatusCode();
    }

    public async Task UpdateOrderAsync(int id, OrderCreateOrUpdateDTO orderDTO)
    {
        var response = await httpClient.PutAsJsonAsync($"https://localhost:7001/api/orders/{id}", orderDTO);
        _ = response.EnsureSuccessStatusCode();
    }

    public async Task UpdateOrderAddProductAsync(int id, ProductDto productDto)
    {
        var response = await httpClient.PostAsJsonAsync($"https://localhost:7001/api/orders/{id}/products", productDto);
        _ = response.EnsureSuccessStatusCode();
    }

    public async Task UpdateOrderDeleteProductAsync(int id, int productId)
    {
        var response = await httpClient.DeleteAsync($"https://localhost:7001/api/orders/{id}/products/{productId}");
        _ = response.EnsureSuccessStatusCode();
    }

    public async Task DeleteOrderAsync(int id)
    {
        var response = await httpClient.DeleteAsync($"https://localhost:7001/api/orders/{id}");
        _ = response.EnsureSuccessStatusCode();
    }
}
