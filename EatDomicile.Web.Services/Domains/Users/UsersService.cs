...
namespace EatDomicile.Web.Services.Domains.Users;
...
public class UsersService : IApiUsersService
{
    ...
    {
        var response = await httpClient.PostAsJsonAsync("https://localhost:7001/api/users", userDTO);
        _ = response.EnsureSuccessStatusCode();
    }

    public async Task UpdateUserAsync(int id, UserDTO userDto)
    {
        var response = await httpClient.PutAsJsonAsync($"https://localhost:7001/api/users/{id}", userDto);
        _ = response.EnsureSuccessStatusCode();
    }

    public async Task DeleteUserAsync(int id)
    {
        var response = await httpClient.DeleteAsync($"https://localhost:7001/api/users/{id}");
        _ = response.EnsureSuccessStatusCode();
    }
}
